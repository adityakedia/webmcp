...
def derive_product_specs(build: dict) -> list[list[str]]:
    ...
    return [
        ...
        ["Damping mass", f'{a["dampingMassG"]} g' if a.get("dampingMassG") else "—"],
        ["Baffle width", f'{a["baffleWidthMm"]} mm' if a.get("baffleWidthMm") else "—"],
        ["Baffle height", f'{a["baffleHeightMm"]} mm' if a.get("baffleHeightMm") else "—"],
        ["Baffle-step adjustment", f'{a["baffleStepDb"]:+g} dB'],
        ["Grille HF adjustment", f'{a["grilleHighFrequencyTrimDb"]:+g} dB'],
