...
export type RoomReferenceAsset = {
  id: string;
  imageUrl: string;
  fileName: string;
  mimeType: string;
...
interface SimulationStore {
  ...
  setSpeakerPosition: (
    side: 'left' | 'right',
    pos: Partial<SimulationStore['speakerPositions']['left']>
  ) => void;
...
export const useSimulationStore = create<SimulationStore>((set) => ({
  ...
  setSelectedSpeaker: (id) =>
    set({ selectedSpeakerId: id, simulationResult: null, simulationStatus: 'queued' }),
  setRoomDimensions: (dims) =>
    set((s) => {
      ...
      return {
        roomDimensions,
        speakerPositions: {
          left: fit(s.speakerPositions.left),
          right: fit(s.speakerPositions.right),
        },
  ...
  setSpeakerPosition: (side, pos) =>
    set((s) => ({
      speakerPositions: { ...s.speakerPositions, [side]: { ...s.speakerPositions[side], ...pos } },
      simulationResult: null,
      simulationStatus: 'queued',
    })),
  setListenerPosition: (pos) =>
    set((s) => ({
      listenerPosition: { ...s.listenerPosition, ...pos },
      simulationResult: null,
      simulationStatus: 'queued',
    })),
  setAudioFile: (file) => set({ audioFile: file }),
  setRoomReferenceAssets: (roomReferenceAssets) => set({ roomReferenceAssets }),
  setSimulationState: (simulationStatus, simulationResult, simulationError = null) =>
    set((state) => ({
      simulationStatus,
  ...
  retrySimulation: () =>
    set((state) => ({ simulationRequestVersion: state.simulationRequestVersion + 1 })),
}));
