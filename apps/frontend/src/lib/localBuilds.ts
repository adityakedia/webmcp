...
const makeId = () =>
  globalThis.crypto?.randomUUID?.() ?? `build-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
export function writeLocalBuilds(value: StoredBuilds) {
  window.localStorage.setItem(
    STORAGE_KEY,
    JSON.stringify({ ...value, builds: value.builds.slice(-MAX_LOCAL_BUILDS) })
  );
...
export function upsertLocalBuild(build: LocalBuild) {
  ...
  writeLocalBuilds({
    ...
    builds: [...builds.filter((item) => item.id !== build.id), build],
  });
}
