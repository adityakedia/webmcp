...
function SpeakerModel({ config }: { config: CustomSpeakerConfiguration }) {
...
export default function ProceduralSpeaker({ config }: { config: CustomSpeakerConfiguration }) { const format = config.brief?.format ?? 'standmount'; const height = (format === 'subwoofer' ? 2.15 : format === 'floorstanding' ? 3.55 : 2.75) * (config.cabinet.size === 'compact' ? .84 : config.cabinet.size === 'large' ? 1.18 : 1); const target: [number, number, number] = [0, height * .48 + .12, 0]; return <Canvas shadows dpr={[1, 1.6]} camera={{ position: [3.75, 2.1, 5.2], fov: 35 }}><color attach="background" args={['#e4e1da']} /><ambientLight intensity={1.15} /><directionalLight position={[3, 6, 5]} intensity={3.4} castShadow shadow-mapSize={[1024, 1024]} /><directionalLight position={[-4, 3, 1]} intensity={.8} /><SpeakerModel config={config} /><mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -.01, 0]} receiveShadow><planeGeometry args={[10, 10]} /><shadowMaterial opacity={.12} /></mesh><ContactShadows position={[0, .01, 0]} opacity={.3} scale={4.5} blur={2.2} far={4} /><Environment preset="studio" /><OrbitControls target={target} enablePan={false} minDistance={3.5} maxDistance={7.5} minPolarAngle={Math.PI / 3.2} maxPolarAngle={Math.PI / 1.85} /></Canvas>; }
