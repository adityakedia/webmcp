...
export function SpeakerModel({ config }: { config: CustomSpeakerConfiguration }) {
  ...
  const bassScale = config.bass.bassCharacter === 'tight' ? .9 : config.bass.bassCharacter === 'extended' ? 1.1 : 1;
  const portRadius = Math.min(width * .16, Math.max(.09, ((config.bass.portInnerDiameterMm ?? 50) / 250) * bassScale)); const portTubeDepth = Math.min(depth * .55, Math.max(.11, (config.bass.portLengthMm ?? 120) / 450));
  const baseLift = config.cabinet.base === 'stand' ? .55 : 0;
  return <group position={[0, .12 + baseLift, 0]}>
    ...
    {config.cabinet.base === 'stand' && <group><mesh position={[0, -.34, 0]} castShadow><cylinderGeometry args={[width * .31, width * .4, .08, 32]} /><meshStandardMaterial color="#242421" roughness={.42} /></mesh><mesh position={[0, -.12, 0]} castShadow><cylinderGeometry args={[.075, .075, .42, 20]} /><meshStandardMaterial color="#292a28" roughness={.38} /></mesh></group>}
    ...
    <Driver position={[0, wooferY + .16, depth / 2 + .1]} radius={(subwoofer ? width * .33 : tall ? .54 : .62) * bassScale} selected={false} onSelect={() => undefined} />
    ...
    {config.personalisation.kind === 'engraving' && <group position={[width / 2 + .014, height * .2 + .16, depth * .2]}><mesh><boxGeometry args={[.018, .08, .18]} /><meshStandardMaterial color="#b99b74" roughness={.45} /></mesh><mesh position={[.012, 0, 0]}><boxGeometry args={[.006, .018, .12]} /><meshStandardMaterial color="#272522" roughness={.5} /></mesh></group>}
    {config.personalisation.kind === 'pattern' && [-.24, 0, .24].map((z) => <mesh key={z} position={[width / 2 + .012, height * .5 + .16, z]}><boxGeometry args={[.012, height * .58, .055]} /><meshStandardMaterial color="#b99b74" roughness={.56} transparent opacity={.72} /></mesh>)}
    {config.personalisation.kind === 'printed_panel' && <RoundedBox args={[.014, height * .52, depth * .62]} radius={.02} smoothness={3} position={[width / 2 + .012, height * .5 + .16, 0]}><meshStandardMaterial color="#47576a" roughness={.56} /></RoundedBox>}
    {config.personalisation.kind === 'decal' && <mesh rotation={[0, Math.PI / 2, 0]} position={[width / 2 + .014, height * .3 + .16, depth * .18]}><circleGeometry args={[.115, 32]} /><meshStandardMaterial color="#d6d1c7" roughness={.42} /></mesh>}
    {config.personalisation.kind === 'custom_artwork' && <RoundedBox args={[.014, height * .62, depth * .72]} radius={.02} smoothness={3} position={[width / 2 + .012, height * .5 + .16, 0]}><meshStandardMaterial color="#7a4e5b" roughness={.46} /></RoundedBox>}
  </group>;
}
